...
import type { JarvisOrientation, JarvisSelectionMode } from '../shared/types';
...
export class JarvisButtonGroup {
  @Prop() label = '';
  @Prop() items = '';
  ...
  private groupId = createId('jarvis-button-group');
  ...
  private moveFocus(items: ReturnType<JarvisButtonGroup['getInteractiveItems']>, direction: 1 | -1) {
    if (!items.length) {
      return;
    }
  ...
  private onKeyDown(event: KeyboardEvent, items: ReturnType<JarvisButtonGroup['getInteractiveItems']>) {
