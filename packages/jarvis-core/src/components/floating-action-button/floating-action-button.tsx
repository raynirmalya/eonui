...
export class JarvisFloatingActionButton {
  ...
  @Event() jarvisAction: EventEmitter<{ value: string; label: string; index: number }>;
  @Event() jarvisToggle: EventEmitter<{ open: boolean }>;
  ...
  private setOpen(next: boolean) {
    ...
    this.jarvisToggle.emit({ open: next });
  }
  ...
  private onMainClick() {
    ...
    if (!actions.length) {
      this.jarvisAction.emit({ value: 'primary', label: this.label, index: -1 });
      return;
    }
  ...
  private onAction(value: string, label: string, index: number) {
    ...
    this.jarvisAction.emit({ value, label, index });
