...
export class JarvisFileManager {
  @Prop() heading = 'Files';
  ...
  @Event() jarvisSelect: EventEmitter<{ path: string; kind: string }>;
  @Event() jarvisPathChange: EventEmitter<{ path: string }>;
  @Event() jarvisAction: EventEmitter<{ action: string; path?: string; paths?: string[]; query?: string }>;
  ...
  async openPath(path: string): Promise<void> {
    ...
    this.jarvisPathChange.emit({ path: nextPath });
  }
  ...
  private setSelection(path: string, kind: string, checked = true) {
    if (this.selectionMode === 'single') {
      ...
      if (checked) {
        this.jarvisSelect.emit({ path, kind });
      }
      return;
    }
            ...
            <li>
              <button
                ...
                onClick={() => {
                  ...
                  this.jarvisPathChange.emit({ path: node.value });
                }}
              >
                <span
                  class="toggle"
            ...
            <tr class={{ active: this.selectedSet.has(row.path) }}>
              <td>
                <input
                  ...
                  name="jarvis-file-manager-selection"
                ...
                <button
                  ...
                  onClick={() => {
                    if (row.kind === 'folder') {
                      ...
                      this.jarvisPathChange.emit({ path: row.path });
                    } else {
  ...
  private renderPreview() {
    ...
    if (!active) {
      return (
        <aside class="preview empty" part="preview">
    ...
    return (
      <aside class="preview" part="preview">
