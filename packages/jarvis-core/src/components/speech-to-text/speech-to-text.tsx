...
export class JarvisSpeechToText {
  ...
  @Event() jarvisResult: EventEmitter<{ value: string; final: boolean }>;
  @Event() jarvisStart: EventEmitter<void>;
  @Event() jarvisEnd: EventEmitter<{ value: string }>;
  @Event() jarvisError: EventEmitter<{ message: string }>;
  @Event() jarvisUnsupported: EventEmitter<void>;
  ...
  private ensureRecognition() {
    ...
    recognition.onstart = () => {
      this.listening = true;
      this.jarvisStart.emit();
    };
    recognition.onend = () => {
      this.listening = false;
      this.interimText = '';
      this.jarvisEnd.emit({ value: this.value });
    };
    recognition.onerror = (event) => {
      this.listening = false;
      this.jarvisError.emit({ message: event.error ?? 'speech-recognition-error' });
    };
  ...
  private handleResult(event: SpeechRecognitionEventLike) {
    ...
    if (finalSegments.length) {
      ...
      this.jarvisResult.emit({ value: this.value, final: true });
    ...
    if (this.interimText) {
      this.jarvisResult.emit({ value: [this.value, this.interimText].filter(Boolean).join(' ').trim(), final: false });
    }
  }
        ...
        <div class="shell" part="base">
          ...
          <div class="footer">
            <span class="status">
              ...
              {this.maxLength > 0 ? ` · ${transcript.length}/${this.maxLength}` : ''}
            </span>
