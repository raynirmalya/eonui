...
export class JarvisToast {
  ...
  @Event() jarvisShow: EventEmitter<void>;
  @Event() jarvisHide: EventEmitter<{ reason: 'dismiss' | 'programmatic' | 'timeout' }>;
  ...
  handleVisibleChange(nextVisible: boolean) {
    ...
    this.jarvisHide.emit({ reason: this.hideReason });
  }
        ...
        >
          ...
          {this.showProgressBar && this.duration > 0 ? (
            <span
              ...
              aria-hidden="true"
              style={{
                animation: `jarvis-toast-progress ${Math.max(this.progressDuration, 1)}ms linear forwards`,
