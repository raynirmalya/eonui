...
export class JarvisDateRangeBox {
  @Prop() label = '';
  ...
  @Event() jarvisChange: EventEmitter<{ start: string; end: string }>;

  private rangeId = createId('jarvis-date-range');
  ...
  private emitChange() {
    this.jarvisChange.emit({ start: this.start, end: this.end });
  }
