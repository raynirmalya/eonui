...
import { renderCloseIcon } from '../shared/icons';
import { createOutsidePointerHandler } from '@eonui/a11y';
import { createId } from '@eonui/utils';
import { matchesSearchTerm, parseOptions, type EonSearchMode } from '../shared/helpers';
...
export class EonAutocomplete {
  ...
  private inputId = createId('eon-autocomplete');
  private listboxId = createId('eon-autocomplete-listbox');
  ...
  private selectValue(nextValue: string) {
    ...
    this.setOpened(false);
    this.eonInput.emit({ value: nextValue });
    this.eonSelect.emit({ value: nextValue });
  }
          ...
          <div class="control-wrap">
            <input
              ...
              onInput={(event) => {
                ...
                this.setOpened(true);
                this.activeIndex = -1;
                this.eonInput.emit({ value: this.value });
              }}
