...
export class EonDropDownButton {
  ...
  private onSelect(optionValue: string, disabled = false) {
    ...
    this.eonSelect.emit({ value: optionValue });
        ...
        <div class="shell" part="base">
          {this.splitButton ? (
            <div class={`split-shell ${this.variant} ${this.size}`} part="trigger">
              ...
              >
                {this.showArrowIcon ? <span class={{ chevron: true, open: this.open }} aria-hidden="true">{renderChevronIcon()}</span> : null}
              </button>
            </div>
          ) : (
            <button
          ...
          <div class={{ panel: true, open: this.open }} part="panel" role="menu" hidden={!this.open} onKeyDown={(event: KeyboardEvent) => this.onPanelKeyDown(event)}>
            {options.map((option, index) => (
              ...
              >
                ...
                {this.showSelectionIndicator && option.value === this.value ? <span class="item-indicator" aria-hidden="true">{renderCheckIcon()}</span> : null}
              </button>
            ))}
          </div>
        </div>
