...
export class EonDropdownMenu {
  ...
  @Event() eonSelect: EventEmitter<{ value: string; label: string }>;
  @Event() eonOpenChange: EventEmitter<{ open: boolean }>;

  private menuId = createId('eon-menu');
  ...
  private openMenu() {
    if (!this.open) {
      this.open = true;
      this.eonOpenChange.emit({ open: true });
    }
    ...
    requestAnimationFrame(() => {
      ...
      this.focusItem(this.activeIndex === -1 ? 0 : this.activeIndex);
    });
  }
  ...
  private closeMenu() {
    ...
    if (this.open) {
      this.open = false;
      this.eonOpenChange.emit({ open: false });
    }
  }
  ...
  private syncViewportPosition() {
    ...
    const alignTop = this.placement.startsWith('top');

    const left = clamp(alignEnd ? triggerRect.right - panelRect.width : triggerRect.left, gap, maxLeft);
    const top = clamp(alignTop ? triggerRect.top - panelRect.height - gap : triggerRect.bottom + gap, gap, maxTop);
  ...
  private selectItem(item: DropdownMenuItemDefinition, index: number) {
    ...
    this.eonSelect.emit({ value: item.value, label: item.label });
    this.closeMenu();
  }
      ...
      <Host>
        ...
        >
          ...
          <div
            ...
            hidden={!this.open}
            tabIndex={-1}
            style={this.renderInViewport ? this.viewportStyle : undefined}
