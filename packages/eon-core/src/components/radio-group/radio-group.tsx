...
export class EonRadioGroup {
  @Prop() label = '';
  @Prop() items = '';
  @Prop({ mutable: true }) value = '';
  @Prop() orientation: EonOrientation = 'vertical';
  @Prop() disabled = false;
  @Prop() readOnly = false;
  @Prop() required = false;
  @Prop() invalid = false;
  ...
  private groupId = createId('eon-radio-group');
  ...
  private moveFocus(items: ReturnType<EonRadioGroup['getItems']>, direction: 1 | -1) {
    if (!items.length) {
      return;
    }
  ...
  private onKeyDown(event: KeyboardEvent, items: ReturnType<EonRadioGroup['getItems']>) {
