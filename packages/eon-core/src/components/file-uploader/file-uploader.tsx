...
export class EonFileUploader {
  ...
  @Event() eonChange: EventEmitter<{ files: string[] }>;
  @Event() eonUpload: EventEmitter<{ files: string[] }>;
  @Event() eonReject: EventEmitter<{ files: string[]; reason: string }>;

  private inputId = createId('eon-file-uploader');
  ...
  private syncFiles(fileList: FileList | null) {
    ...
    const reject = (files: File[], reason: string) => {
      ...
      this.eonReject.emit({ files: files.map((file) => file.name), reason });
    };
    ...
    this.eonChange.emit({ files: this.files.map((file) => file.name) });
    ...
    const timer = window.setInterval(() => {
      ...
      if (nextProgress >= 100) {
        ...
        if (this.files.every((file) => file.status === 'uploaded')) {
          ...
          this.eonUpload.emit({ files: this.files.map((file) => file.name) });
        }
      }
    }, 180);
