...
function createTasks(label: string): Array<{ title: string; meta: string; tone: string }> {
  return [
    ...
    { title: `${label} sync`, meta: 'Assigned to Eon team', tone: 'success' },
...
export class EonTabPanel {
  ...
  @Event() eonChange: EventEmitter<{ index: number; label: string }>;
  ...
  async select(index: number): Promise<void> {
    ...
    this.current = index;
    this.eonChange.emit({ index, label: tabs[index] });
  }
  ...
  private onSelect(index: number, label: string): void {
    ...
    this.current = index;
    this.eonChange.emit({ index, label });
  }
