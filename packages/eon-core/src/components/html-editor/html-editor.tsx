...
<p>Eon HTML Editor helps you format text, create lists, and embed links or media without leaving the design system.</p>
<ul>
  <li>Inline emphasis and headings</li>
  <li>Links, lists, and block quotes</li>
  <li>Lightweight media insertion</li>
</ul>
<blockquote>Use Eon HTML Editor for editorial experiences, settings pages, and knowledge workflows.</blockquote>
`;
...
export class EonHtmlEditor {
  ...
  private syncFromEditor() {
    ...
    this.eonChange.emit({ value: this.html });
  }
  ...
  private insertLink() {
    this.applyCommand('createLink', 'https://eon-ui.dev/docs');
  }

  private insertImage() {
    this.applyCommand('insertHTML', '<figure><img src="https://placehold.co/640x320/e2e8f0/0f172a?text=Eon+Media" alt="Inserted media" /></figure>');
  }

  private insertCodeBlock() {
    this.applyCommand('insertHTML', '<pre><code>const workspace = { name: "Eon UI", stage: "review" };</code></pre>');
  }
      ...
      <Host>
        <div class="shell" part="base">
            ...
            <div class="toolbar" part="toolbar">
              ...
              {this.toolbarPreset === 'full' ? (
                <div class="group">
                  <select
                    ...
                    onInput={(event) => this.applyCommand('fontName', (event.target as HTMLSelectElement).value)}
                    disabled={locked || this.sourceMode}
                  >
                  ...
                  <select
                    ...
                    onInput={(event) => this.applyCommand('fontSize', (event.target as HTMLSelectElement).value)}
                    disabled={locked || this.sourceMode}
                  >
              ...
              <div class="group">
                <select
                  ...
                  onInput={(event) => this.applyCommand('formatBlock', (event.target as HTMLSelectElement).value)}
                  disabled={locked || this.sourceMode}
                >
          ...
          <div class="footer" part="footer">
            <span>{this.textLength} characters{this.showWordCount ? ` Â· ${this.wordCount} words` : ''}</span>
