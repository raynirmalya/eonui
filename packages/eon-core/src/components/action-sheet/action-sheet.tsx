...
import { createOutsidePointerHandler, restoreFocus } from '@eonui/a11y';
...
export class EonActionSheet {
  ...
  render() {
    ...
    return (
      <Host>
        <div class={{ overlay: true, open: this.open, fullscreen: this.fullScreen }} part="overlay" hidden={!this.open} aria-hidden={this.open ? 'false' : 'true'} onClick={(event) => this.onBackdropClick(event)}>
          <section
            class={{ panel: true, [this.presentation]: true, fullscreen: this.fullScreen }}
            part="panel"
            role="dialog"
            aria-modal="true"
