...
export class EonStepper {
  ...
  @Event() eonStepChange: EventEmitter<{ index: number; label: string }>;
  ...
  private requestStep(index: number, item: StepItemDefinition) {
    ...
    this.eonStepChange.emit({ index, label: item.label });
  }
  ...
  render() {
    ...
    return (
      <Host>
        <div class={{ stepper: true, vertical: this.orientation === 'vertical' }} part="base" role="group" aria-label={this.ariaLabel}>
