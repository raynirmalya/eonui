...
import { focusPart, restoreFocus, trapFocus } from '@eonui/a11y';
...
export class EonDialog {
  ...
  @Event() eonOpen: EventEmitter<void>;
  @Event() eonClose: EventEmitter<{ reason: 'dismiss' | 'programmatic' }>;
  ...
  handleOpenChange(nextOpen: boolean) {
    ...
    this.eonClose.emit({ reason: this.closeReason });
  }
  ...
  render() {
    ...
    return (
      <Host onKeyDown={this.onKeyDown}>
        ...
        >
          <section
            class={{ panel: true, [this.size]: true, fullscreen: this.fullScreen }}
            part="panel"
            role="dialog"
            aria-modal="true"
