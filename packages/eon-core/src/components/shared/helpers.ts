...
export type EonNumericFormat = 'decimal' | 'integer' | 'fixed-point' | 'currency' | 'accounting' | 'percent' | 'unit';

export interface EonNumericFormatterConfig {
...
export type EonSearchMode = 'contains' | 'startsWith' | 'startswith' | 'equals';
export type NormalizedEonSearchMode = 'contains' | 'startsWith' | 'equals';
...
export function normalizeSearchMode(mode: EonSearchMode | string | undefined): NormalizedEonSearchMode {
...
export function matchesSearchTerm(value: string, query: string, mode: EonSearchMode = 'contains'): boolean {
  if (!query) {
    return true;
  }
...
export function formatNumericValue(value: number, config: EonNumericFormatterConfig = {}): string {
