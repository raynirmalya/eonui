...
export class EonAccordion {
  ...
  @Event() eonToggle: EventEmitter<{ value: string; open: boolean }>;
  @Event() eonChange: EventEmitter<{ values: string[] }>;

  private baseId = createId('eon-accordion');
  ...
  private toggleSingle() {
    ...
    this.eonToggle.emit({ value: this.summary, open: nextOpen });
  ...
  private toggleItem(item: AccordionItemDefinition) {
    ...
    this.eonToggle.emit({ value: item.value, open: !expanded });
  ...
  private renderChevronIcon() {
    return (
      <svg class="chevron-icon" viewBox="0 0 16 16" focusable="false">
        <path class="chevron-path" d="M5 3.75L10 8L5 12.25" />
      </svg>
    );
  }
